...
function birthdate(balise) {
  ...
  const birthdateRegExp = RegExp(/^\d{4}-\d{2}-\d{2}$/);
  viderBaliseErreur(balise);
  try {
...
function validerNbr(balise) {
  ...
  const nbrRegExp = new RegExp('^[0-9]\\d*$');
  viderBaliseErreur(balise);
  try {
...
function validerRadio(balise, nomBalise) {
  ...
  const baliseCheckBox = document.querySelectorAll(`input[name="${balise}"]`);
  let check = '';
...
function validerCheckBox(balise) {
  ...
  const baliseCheckBox = document.getElementById(`${balise}`).checked;
  viderBaliseErreur(balise);
  try {
    if (!baliseCheckBox) {
